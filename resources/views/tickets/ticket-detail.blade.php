                ...
                <div class="panel-container show">
                    <div class="panel-content poisition-relative">
                        ...
                        <h5 class="card-title">Contact: <span class="text-primary">{{ $data_ticket->name }} ,</span>
                            <span
                                class="ml-3">{{ \Carbon\Carbon::parse($data_ticket->created_at)->format('d-M-Y h:i A') ?? '' }}</span>
                        </h5>
                        <p class="card-text">Issue Type:
                            {{ $data_ticket->issueType ? $data_ticket->issueType->name : $data_ticket->custom1 }}</p>
                        <p class="card-text">
                            {!! $data_ticket->message !!}
                            {{-- {!! nl2br(e($data_ticket->message)) !!} --}}
                        </p>
                        <p class="card-text"><a href="{{ url('storage/attachments', $data_ticket->attachments) }}"
                                target="_blank">{{ $data_ticket->attachments == 'undefined' ? '' : $data_ticket->attachments }}</a>
                        </p>
                        ...
                        <div class="form-noted mt-3" style="display: none;">
                            <div class="form-group">
                                <label class="form-label" for="ticket-textarea">Message: <span
            ...
            <div id="panel-2" class="panel panel-sortable" role="widget">
                ...
                <div class="panel-container show">
                    <div class="panel-content poisition-relative">
                        ...
                        @if (count($responses_tickets) > 0)
                            <br>
                            <div class="btn-group-vertical" role="group" aria-label="Vertical button group" style="float: right;">
                                    <button id="btnGroupVerticalDrop1" type="button" class="btn dropdown-toggle waves-effect waves-themed" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        ...
                                        <div class="demo" style="display: flex">
                                            <div class="custom-control custom-checkbox ">
                                                <input type="checkbox" class="custom-control-input" id="autostart"
                                                    checked="">
                                                <label class="custom-control-label mr-3" for="autostart">Attach signature (
                                                    Profile settings )</label>
                                            </div>
                                        </div>
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input"
                                                id="autoreload-send-email" name="autoreload-send-email">
                                            <label class="custom-control-label" for="autoreload-send-email">Don't send
                                                email notification of this reply to the customer</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group form-group-select2">
                                        ...
                                        <select
                                            class="select2 form-control w-100 select2-hidden-accessible required select2-option"
                                            id="ticket-status" required>
                                            <option value=""></option>
                                            @if (count($status) > 0)
                                                @foreach ($status as $item)
                                                    <option @if ($item->id == $data_ticket->status) selected @endif
                                    ...
                                    <div class="form-group form-group-select2">
                                        ...
                                        <select
                                            class="select2 form-control w-100 select2-hidden-accessible required select2-option"
                                            id="ticket-priority" required>
                                            <option value=""></option>
                                            @if (count($priority) > 0)
                                                @foreach ($priority as $item)
                                                    <option @if ($item->id == $data_ticket->priority) selected @endif
                                                        style="color: {{ $item->color }}" value="{{ $item->id }}">
                                                        {{ $item->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
        ...
        <div class="col-md-4">
            <div class="btn-group btn-group-custom d-flex justify-content-end" role="group" aria-label="Print Options">
                ...
                <div class="dropdown-menu">
                    <a class="dropdown-item" href="#"><i class="fal fa-envelope"></i> Re-send email
                        notification</a>
                    @can('Ticket Export')
                ...
                <div class="accordion" id="accordionExample">
                    <div class="card">
                        ...
                        <div id="collapseOne" class="collapse show" aria-labelledby="headingOne"
                            ...
                            <div class="card-body">
                                <p class="card-text">Tracking ID: <strong
                                        class="ml-3">{{ $data_ticket->trackid }}</strong></p>
                                <p class="card-text">Ticket number: <strong
                                        class="ml-3">{{ $data_ticket->id }}</strong></p>
                                <p class="card-text">Created on: <strong
                                        class="ml-3">{{ \Carbon\Carbon::parse($data_ticket->created_at)->format('d-M-Y h:i A') ?? '' }}</strong>
                                </p>
                                <p class="card-text">Updated: <strong
                                        class="ml-3">{{ \Carbon\Carbon::parse($data_ticket->updated_at)->format('d-M-Y h:i A') ?? '' }}</strong>
                                </p>
                                <p class="card-text">Replies: <strong class="ml-3" id="total_replies">0</strong></p>
                                <p class="card-text">Last replier: <strong
                                        class="ml-3">{{ $data_ticket->lastReplier ? $data_ticket->lastReplier->name : '' }}</strong>
                                </p>
                                {{-- <p class="card-text">Time worked: <strong class="ml-3">00:00</strong></p> --}}
                                <p class="card-text">Due date: <strong
                                        class="ml-3">{{ \Carbon\Carbon::parse($data_ticket->due_date)->format('d-M-Y') ?? '' }}</strong>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="accordion" id="History">
                    <div class="card">
                        <div class="card-header" id="headingThree">
                            <a href="javascript:void(0);" class="card-title collapsed show" data-toggle="collapse"
                                            ...
                                            @if ($item->createdBy)
                                                ...
                                                @if ($item->type == 'status')
                                                    <li><strong>Status changed </strong>
                                                        <ul style="list-style-type:none;">
                                                            <li>From <strong
                                                                    style="color: {{ $item->statusFrom->color }}">{{ $item->statusFrom->name }}</strong>
                                                                to <strong
                                                                    style="color: {{ $item->statusTo->color }}">{{ $item->statusTo->name }}</strong>
                                                                by user change {{ $item->createdBy->user }} at
                                                                {{ \Carbon\Carbon::parse($item->created_at)->format('d-M-Y h:i A') ?? '' }}
                                                            </li>
                                                        </ul>
                                                    </li>
                                                ...
                                                @if ($item->type == 'priority')
                                                    <li><strong>Priority changed </strong>
                                                        <ul style="list-style-type:none;">
                                                            <li>From <strong
                                                                    style="color: {{ $item->priorityFrom->color }}">{{ $item->priorityFrom->name }}</strong>
                                                                to <strong
                                                                    style="color: {{ $item->priorityTo->color }}">{{ $item->priorityTo->name }}</strong>
                                                                by user change {{ $item->createdBy->user }} at
                                                                {{ \Carbon\Carbon::parse($item->created_at)->format('d-M-Y h:i A') ?? '' }}
                                                            </li>
                                                        </ul>
                    ...
                    <div class="modal-btn delete-action">
                        <form>
                            ...
                            <div class="float-lg-right">
                                ...
                                <button type="button" class="btn btn-danger waves-effect waves-themed btn-delete-ticket"
                                    data-id="{{ $data_ticket->id }}">Delete</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $(document).ready(function () {
                $('form.form-edit-reply').on('submit', function (e) {
                    let summernoteContent = $('#e_ticket-reply').summernote('code');
                    $('#e_reply_message').val(summernoteContent);
                });
            });
                    ...
                    if (datas.length > 0) {
                        datas.forEach(function(value, index) {
                            ...
                            if (userPermissions.includes('Ticket Delete Note')) {
                                text +=
                                    '<a style="float: right;" href="javascript:void(0);" id="btn-note-delete" data-id="' +
                                    value.id +
                                    '" data-toggle="tooltip" title="Delete" class="btn btn-outline-primary btn-sm btn-icon waves-effect waves-themed">' +
                                    '<i class="fal fa-trash-alt"></i>' +
                                    '</a>';
                            }
                            if (userPermissions.includes('Ticket Edit Note')) {
                                text +=
                                    '<a style="float: right;" href="javascript:void(0);" id="btn-note-edit" data-id="' +
                                    value.id + '" data-message="' + value.message +
                                    '" data-toggle="tooltip" title="Edit" class="mr-1 btn btn-outline-secondary btn-sm btn-icon waves-effect waves-themed">' +
                                    '<i class="fal fa-edit"></i>' +
                                    '</a>';
                            }
                            text += '<p class="card-text">Note by: <strong>' + value.created_by.user +
                                '</strong> » ' + created_at + '</p>' +
                                '</div>' +
                            ...
                            note_tr += '<tr>' +
                                '<td class="table_tr">' +
                                '<strong>Note by: ' + value.created_by.user + '</strong> » ' +
                                created_at + '<br>' +
                                message +
                                '</td>' +
                                '</tr>';
                    ...
                    if (datas.length > 0) {
                        ...
                        datas.forEach(function(value, index) {
                            ...
                            if (userPermissions.includes('Ticket Delete Reply')) {
                                btn_delete =
                                    '<a class="dropdown-item" href="javascript:void(0);" id="btn-reply-delete" data-id="' +
                                    value.id + '"><i class="fal fa-trash-alt"></i> Delete reply</a>';
                            }
                            text += '<div class="panel-tag">' +
                                '<div>' +
                            ...
                            text += '<p class="card-text">Reply by: <strong>' + value.staff.user +
                                ...
                                '</div>' +
                                '<p class="card-text mt-2">' + message + '</p>' +
                                '<p class="card-text"><a href="{{ url('storage/attachments') }}/' + (
                                    value.attachments) + '" target="_blank">' + value.attachments +
                                '</a></p>' +
                                '</div>';
                            reply_tr += '<tr>' +
                                '<td class="table_tr">' +
                                '<strong>Reply by: ' + value.staff.user + '</strong> » ' + created_at +
                                '<br>' +
                                message +
                                '</td>' +
                                '</tr>';
